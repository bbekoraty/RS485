...
void MyModbusSensor::readSensorValues(uint8_t startAddress, uint8_t numRegisters)
{
  ...
  if (startAddress == 0&&numRegisters == 2)//Conductivity Address 
  {
  ...
  else if (startAddress == 2&&numRegisters == 2)//Temperature Address
  {
    result = _node.readInputRegisters(startAddress, numRegisters);
    if (result == _node.ku8MBSuccess)
    {
  ...
  //   else if (startAddress == 2) //Addition Read Function
  // {
